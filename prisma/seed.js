...
async function main() {
  await prisma.publication.createMany({
    data: [
      ...englishPublications.map((publication) => ({ ...publication, language: "en" })),
      ...arabicPublications.map((publication) => ({ ...publication, language: "ar" }))
    ]
  });

  await prisma.mediaItem.createMany({
    data: [
      ...mediaItems.en.map((item) => ({ ...item, language: "en" })),
      ...mediaItems.ar.map((item) => ({ ...item, language: "ar" }))
    ]
  });
}
